// Yahoo Delta: how much later (or earlier) Yahoo's rankings rank a player
// vs. your own board. Positive = value steal (Yahoo ranks them later, safe
// to wait); negative = overdraft trap (Yahoo ranks them earlier than you
// do). Duplicate of lib/sleeperDelta.ts for the Yahoo ADP/rankings source —
// see that file for the Sleeper equivalent.
...
export function yahooDeltaTier(roundsToWait: number): DeltaTier {
...
// Row-tint class for a player, or '' if they have no Yahoo ADP data (in
// which case the caller should fall back to the table's normal row color).
export function yahooRowTint(
  yourRank: number,
  yahooRank: number | null,
  teams: number,
  currentPick: number
): string {
  if (yahooRank == null) return '';
  const { tier } = yahooDelta(yourRank, yahooRank, teams, currentPick);
...
//
// Once live picks pass a player's *Yahoo* ADP and they're still on the
// board, that's confirmed evidence the real draft disagrees with Yahoo —
...
export function yahooDelta(
  ...
  if (currentPick > yahooRank) {
    const delta = currentPick - yahooRank;
    const roundsToWait = Math.floor(delta / size);
    return {
      delta,
      roundsToWait,
      tier: yahooDeltaTier(roundsToWait),
      pastAdp: true,
    };
  }

  const referenceRank = Math.max(yourRank, currentPick);
  const delta = yahooRank - referenceRank;
  const roundsToWait = Math.floor(delta / size);
  return {
    delta,
    roundsToWait,
