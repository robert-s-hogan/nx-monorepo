// ESPN Delta: how much later (or earlier) ESPN's rankings rank a player vs.
// your own board. Positive = value steal (ESPN ranks them later, safe to
// wait); negative = overdraft trap (ESPN ranks them earlier than you do).
// Duplicate of lib/yahooDelta.ts for the ESPN ADP/rankings source — see
// that file for the Yahoo equivalent.
...
export function espnDeltaTier(roundsToWait: number): DeltaTier {
...
// Row-tint class for a player, or '' if they have no ESPN ADP data (in
// which case the caller should fall back to the table's normal row color).
export function espnRowTint(
  yourRank: number,
  espnRank: number | null,
  teams: number,
  currentPick: number
): string {
  if (espnRank == null) return '';
  const { tier } = espnDelta(yourRank, espnRank, teams, currentPick);
...
//
// Once live picks pass a player's *ESPN* ADP and they're still on the
// board, that's confirmed evidence the real draft disagrees with ESPN —
...
export function espnDelta(
  ...
  if (currentPick > espnRank) {
    const delta = currentPick - espnRank;
    const roundsToWait = Math.floor(delta / size);
    return {
      delta,
      roundsToWait,
      tier: espnDeltaTier(roundsToWait),
      pastAdp: true,
    };
  }

  const referenceRank = Math.max(yourRank, currentPick);
  const delta = espnRank - referenceRank;
  const roundsToWait = Math.floor(delta / size);
  return {
    delta,
    roundsToWait,
