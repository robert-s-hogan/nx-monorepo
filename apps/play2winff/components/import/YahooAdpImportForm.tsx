...
import { replaceYahooAdp, useYahooAdpStatus } from '../../hooks/useYahooAdp';

export const YahooAdpImportForm = () => {
  ...
  async function commit() {
    ...
    setError(null);
    try {
      const result = await replaceYahooAdp(goodRows);
      if ('error' in result) {
    ...
    <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {hasParsed && (
        <div className="flex flex-wrap items-center gap-4 text-sm">
          ...
          >
            {committing ? 'Saving…' : 'Update Yahoo ADP'}
          </button>
        </div>
      )}
      ...
      {committed && (
        <p className="text-sm font-medium text-emerald-600">
          ✓ Yahoo ADP updated successfully.
        </p>
      )}
